...
class GridEmbeddingND(nn.Module):
    ...
    def grid(self, spatial_dims: torch.Size, device: str, dtype: torch.dtype):
        ...
        if (
            self._grid is None
            or self._res != spatial_dims
            or self._grid[0].device != torch.device(device)
        ):
...
class SinusoidalEmbedding(Embedding):
    ...
    def forward(self, x):
        ...
        if self.embedding_type == "nerf":
            freqs = (
                2 ** torch.arange(0, self.num_frequencies, device=x.device) * torch.pi
            )

        elif self.embedding_type == "transformer":
            freqs = torch.arange(0, self.num_frequencies, device=x.device) / (
...
class RotaryEmbedding2D(nn.Module):
    ...
    def forward(self, coordinates):
        ...
        freqs = torch.einsum(
            "... i , j -> ... i j", coordinates, self.inv_freq
        )  # [b, n, d//2]
